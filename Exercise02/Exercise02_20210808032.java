...
public class Exercise02_20210808032 {
    ...
    public static char[] grade(int[] students) {
        ...
        // Your code goes here...
        int trash=0;
        char[] lettergrade = new char[students.length];
            trash = getMax(students);
         
            for (int i = 0; i< students.length; i++) {
                if (students[i] >= trash- 10) {
                    lettergrade[i] = 'A';
                } else if (students[i]>=trash-20) {
                    lettergrade[i] = 'B';
                } else if (students[i]>=trash-30) {
                    lettergrade[i] = 'C';
                } else if (students[i]>=trash-40) {
                    lettergrade[i] = 'D';
                } else {
                    lettergrade[i] = 'F';
                }
    ...
    public static int random(int start, int end, int... exclude) {
         ...
         */
         ...
         // Your code goes here...
         int ran = (int)(Math.random() * (end-start+ 1));
            for (int i = 0; i < exclude.length; i++) {
                    
            if (ran == exclude[i]) {
                 ran = (int)(Math.random() *(end-start+ 1));
                 i = -1;
             }
         }
         return ran;
 ...
 // Question 4: Eliminate duplicates
    ...
    public static int[] lockers(boolean[] locker) {
        ...
        // Your code goes here...
            ...
            int[] oplock= new int[locker.length];
            int opentemp = 0;
            ...
            int[] finalresult = new int[opentemp];
                
            for (int i = 0; i < opentemp; i++) {
                    finalresult[i] = oplock[i];
                }
            return finalresult;
          }
