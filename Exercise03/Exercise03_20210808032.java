...
public class Exercise03_20210808032 {
    /*
    ...
    public static int[] employeeWorkingHours(int[][] employe) {
        /*
        ...
        {
            int temp=finalr[i];
            finalr[i] = finalr[finalr.length-i-1];
            finalr[finalr.length-i-1] = temp;
        }
    ...
    public static int[][] dotProduct(int[][] matrixA, int[][] matrixB) {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < k; j++) {
                for (int l = 0; l < n; l++) {
                    productMatrix[i][j] += matrixA[i][l] * matrixB[l][j];
                }
            }
     }
    ...
    public static int[] getClosestPoints(double[][] points, int pointIndex) {
        ...
        // Your code goes here...
            ...
            for (int i = 0; i < points.length; i++) {
                distances[i] = distance(points[i], points[pointIndex]);
            }
            ...
            int[] closestPoints = new int[count];
            int j = 0;
            for (int i = 0; i < distances.length; i++) {
                if (distances[i] == minDistance) {
                    closestPoints[j] = i;
    ...
    public static double distance(double[] pointA, double[] pointB) {
        ...
        if (pointA.length != pointB.length) {
            throw new IllegalArgumentException("Points should be same number of dimensions");
        }
